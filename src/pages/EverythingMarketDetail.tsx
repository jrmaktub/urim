...
export default function EverythingMarketDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  const marketId = id ? parseInt(id) : 0;
  
  const market = useMarketInfo(marketId, false);
      ...
      <main className="container mx-auto px-4 py-24">
        ...
        <div className="grid gap-4 mb-8">
          ...
          {market.outcomes.map((outcome, idx) => {
            const pool = useOutcomePool(marketId, idx, false);
      ...
      <Dialog open={showBetModal} onOpenChange={setShowBetModal}>
        <DialogContent className="glass-card border-primary/20">
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-3">
              <Label className="text-base font-semibold">Select Outcome</Label>
