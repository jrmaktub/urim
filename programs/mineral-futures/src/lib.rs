...
pub const MAX_POSITION_VAULT_BPS: u64 = 2000; // max 20% of vault per position
pub const FUNDING_INTERVAL: i64 = 28800; // 8 hours
pub const MAX_FUNDING_RATE_BPS: i64 = 10; // 0.1% per interval
pub const MIN_COLLATERAL: u64 = 10_000_000; // 0.01 SOL
pub const MAX_PRICE_AGE: i64 = 21600; // 6 hours
...
pub mod mineral_futures {
    ...
    pub fn open_position(
        ...
        // Auto-crank funding (free, no bot needed)
        try_apply_funding(&mut ctx.accounts.market)?;
        ...
        require!(now - market.last_price_update <= MAX_PRICE_AGE, FuturesError::StalePrice);
        ...
        if total_oi > 0 {
            ...
            let max_notional = vault_balance
                .checked_mul(MAX_POSITION_VAULT_BPS).unwrap_or(0)
                .checked_div(BPS_DENOMINATOR).unwrap_or(0);
        ...
        let base_fee = collateral_lamports
            .checked_mul(TAKER_FEE_BPS).unwrap_or(0)
            .checked_div(BPS_DENOMINATOR).unwrap_or(0);

        let fee = if verified_urim {
            base_fee
                .checked_mul(URIM_DISCOUNT_NUMERATOR).unwrap_or(base_fee)
                .checked_div(URIM_DISCOUNT_DENOMINATOR).unwrap_or(base_fee)
        } else {
            base_fee
        };
        ...
        position.entry_funding_rate = ctx.accounts.market.funding_rate_cumulative;
    ...
    pub fn close_position(ctx: Context<ClosePosition>) -> Result<()> {
        ...
        let pnl = calc_pnl(position.direction, market.mark_price, position.entry_price, collateral, leverage);
        let funding = calc_funding_payment(
            position.direction, market.funding_rate_cumulative, position.entry_funding_rate, collateral,
        );
        let total_pnl = pnl.saturating_add(funding);
        ...
        let actual_payout = payout.min(vault_balance.saturating_sub(rent_min));
    ...
    pub fn liquidate(ctx: Context<Liquidate>) -> Result<()> {
        ...
        let threshold = (collateral as i64)
            .checked_mul(9000).unwrap_or(i64::MAX)
            .checked_div(BPS_DENOMINATOR as i64).unwrap_or(i64::MAX);
        require!(total_pnl <= -threshold, FuturesError::NotLiquidatable);

        let liquidator_reward = collateral
            .checked_mul(LIQUIDATOR_REWARD_BPS).unwrap_or(0)
            .checked_div(BPS_DENOMINATOR).unwrap_or(0);

        let vault_balance = ctx.accounts.vault.to_account_info().lamports();
        let rent_min = Rent::get()?.minimum_balance(8);
        let actual_reward = liquidator_reward.min(vault_balance.saturating_sub(rent_min));
    ...
    /// Manual funding crank (kept for backwards compat, but auto-crank handles it)
    pub fn apply_funding(ctx: Context<ApplyFunding>) -> Result<()> {
        let market = &mut ctx.accounts.market;
        let now = Clock::get()?.unix_timestamp;
        require!(now - market.last_funding_time >= FUNDING_INTERVAL, FuturesError::FundingTooEarly);
        do_apply_funding(market, now);
        Ok(())
    }
    ...
    pub fn withdraw_fees(ctx: Context<WithdrawFees>, amount: u64) -> Result<()> {
        ...
        let total_oi = market.open_interest_long.saturating_add(market.open_interest_short);
        let min_vault = total_oi.saturating_mul(2).saturating_add(rent_min);
...
/// Auto-crank: silently applies funding if interval has passed. No error if too early.
fn try_apply_funding(market: &mut Account<Market>) -> Result<()> {
    let now = Clock::get()?.unix_timestamp;
...
fn do_apply_funding(market: &mut Account<Market>, now: i64) {
    ...
    let rate: i64 = if total_oi > 0 {
        oi_long.saturating_sub(oi_short)
...
fn verify_urim_balance(remaining_accounts: &[AccountInfo], trader: &Pubkey) -> Result<bool> {
    if remaining_accounts.is_empty() { return Ok(false); }
    let acct = &remaining_accounts[0];
    if acct.owner.to_bytes() != SPL_TOKEN_PROGRAM { return Ok(false); }
    let data = acct.try_borrow_data()?;
    if data.len() < 72 { return Ok(false); }
    if data[0..32] != URIM_MINT || data[32..64] != trader.to_bytes() { return Ok(false); }
